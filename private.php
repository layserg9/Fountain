<?php
/*
Template Name: private
*/
?>
<?php get_header('fountains'); ?>


      <div class="upBotton__body" onclick="scrollTopTop()">
      </div>
      <main class="page">
         <div class="page__main-block main-block">
            <div class="main-block__container _container">
               <div class="main-block__body">
                  <h1 class="main-block__title">Частные фонтаны</h1>
                  <div class="main-block__buttons">
                     <a data-goto=".page__subscribe" href="#"
                        class="main-block__button main-block__button_orange">Оставить заявку</a>
                  </div>
               </div>

            </div>
            <div class="main-block__image _ibg _other">
               <img src="<?php echo get_template_directory_uri(); ?>/img/private-backer.jpg" alt="Фонтан вашей мечты">
            </div>
         </div>
         <section class="page__services services">
            <div class="services__container _container">
               <div class="services__body">
                  <div class="services__column">
                     <a href="/city/" class="services__link">
                        <div class="services__item item-service">
                           <div class="item-service__icon">
                              <svg version="1.1" id="Layer_1" fill="currentColor" alt="Городской фонтан"
                                 xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
                                 y="0px" viewBox="0 0 511.999 511.999" xml:space="preserve">

                                 <path d="M124.759,141.457c4.641,0,8.401-3.76,8.401-8.401v-24.909c0-4.641-3.76-8.401-8.401-8.401s-8.401,3.76-8.401,8.401
				v24.909C116.358,137.697,120.118,141.457,124.759,141.457z" />
                                 <path
                                    d="M124.759,82.42c4.641,0,8.401-3.76,8.401-8.401c0-31.549,25.666-57.217,57.215-57.217
				c31.55,0,57.219,25.667,57.219,57.217c0,4.641,3.76,8.401,8.401,8.401c4.641,0,8.401-3.76,8.401-8.401
				c0-31.549,25.668-57.217,57.219-57.217c31.549,0,57.215,25.667,57.215,57.217c0,4.641,3.76,8.401,8.401,8.401
				c4.64,0,8.401-3.76,8.401-8.401C395.633,33.205,362.43,0,321.616,0c-28.487,0-53.249,16.183-65.62,39.831
				C243.624,16.183,218.862,0,190.375,0c-40.813,0-74.018,33.205-74.018,74.019C116.358,78.66,120.118,82.42,124.759,82.42z" />
                                 <path d="M380.37,121.568c-4.641,0-8.401,3.76-8.401,8.401v8.735c0,4.641,3.76,8.401,8.401,8.401c4.641,0,8.401-3.76,8.401-8.401
				v-8.735C388.771,125.329,385.011,121.568,380.37,121.568z" />
                                 <circle cx="380.37" cy="412.657" r="9.214" />
                                 <circle cx="131.628" cy="412.657" r="9.214" />
                                 <path d="M490.977,461.206h-24.515v-105.06h3.171c11.513,0,20.88-9.367,20.88-20.881v-9.425c0-11.513-9.366-20.88-20.88-20.88
				h-98.639c13.421-6.852,25.481-14.926,35.797-24.034c4.444-3.923,8.523-8.033,12.272-12.288h13.246
				c4.641,0,8.401-3.76,8.401-8.401s-3.76-8.401-8.401-8.401h-0.779c6.024-9.799,10.376-20.171,12.936-30.909
				c10.505-1.081,18.726-9.983,18.726-20.769v-9.592c0-11.513-9.366-20.881-20.88-20.881H303.992
				c3.922-7.606,6.056-16.108,6.056-24.781c0-29.804-24.247-54.053-54.052-54.053c-29.804,0-54.052,24.248-54.052,54.053
				c0,8.674,2.134,17.177,6.054,24.781H69.685c-11.513,0-20.88,9.367-20.88,20.881v9.592c0,10.787,8.221,19.688,18.727,20.769
				c2.56,10.738,6.911,21.109,12.936,30.909h-0.783c-4.641,0-8.401,3.76-8.401,8.401s3.762,8.401,8.401,8.401h13.249
				c3.745,4.251,7.821,8.356,12.259,12.276c10.318,9.114,22.382,17.191,35.81,24.046H42.364c-11.513,0-20.88,9.366-20.88,20.88
				v9.425c0,11.513,9.366,20.881,20.88,20.881h3.17v105.06H21.02c-10.88,0-19.73,8.85-19.73,19.73v11.332
				c0,10.88,8.85,19.73,19.73,19.73h469.957c10.881,0,19.733-8.85,19.733-19.73v-11.332
				C510.71,470.058,501.858,461.206,490.977,461.206z M255.996,107.655c20.539,0,37.25,16.711,37.25,37.251
				c0,9.277-3.338,17.958-9.45,24.781h-55.602c-6.109-6.823-9.447-15.505-9.447-24.781
				C218.746,124.366,235.456,107.655,255.996,107.655z M69.685,204.239c-2.21,0-4.077-1.867-4.077-4.078v-9.592
				c0-2.211,1.867-4.078,4.077-4.078h372.628c2.21,0,4.077,1.867,4.077,4.078v9.592c0,2.211-1.867,4.078-4.077,4.078H69.685z
				 M100.833,251.837c-7.401-9.632-12.749-20.003-15.896-30.797h342.127c-3.149,10.794-8.496,21.165-15.897,30.797H100.833z
				 M116.689,268.64h278.619c-18.296,16.038-42.806,28.579-70.971,36.322H187.658C159.494,297.217,134.985,284.679,116.689,268.64z
				 M42.364,339.344c-2.21,0-4.077-1.867-4.077-4.078v-9.425c0-2.21,1.867-4.077,4.077-4.077h427.269
				c2.21,0,4.077,1.867,4.077,4.077v9.425c0,2.211-1.867,4.078-4.077,4.078H42.364z M449.66,356.147v105.06H62.337v-105.06H449.66z
				 M493.907,492.27c0,1.587-1.342,2.928-2.93,2.928H21.02c-1.587,0-2.928-1.341-2.928-2.928v-11.332
				c0-1.587,1.341-2.928,2.928-2.928h469.957c1.588,0,2.93,1.341,2.93,2.928V492.27z" />
                                 <path d="M131.628,450.176c20.687,0,37.517-16.83,37.517-37.517c0-20.688-16.83-37.519-37.517-37.519
				c-20.688,0-37.519,16.83-37.519,37.519C94.109,433.345,110.939,450.176,131.628,450.176z M131.628,391.941
				c11.422,0,20.715,9.294,20.715,20.716s-9.293,20.715-20.715,20.715c-11.422,0-20.716-9.293-20.716-20.715
				S120.205,391.941,131.628,391.941z" />
                                 <path d="M380.37,450.176c20.687,0,37.518-16.83,37.518-37.517c0-20.688-16.83-37.519-37.518-37.519
				c-20.687,0-37.517,16.83-37.517,37.519C342.853,433.345,359.684,450.176,380.37,450.176z M380.37,391.941
				c11.422,0,20.715,9.294,20.715,20.716s-9.293,20.715-20.715,20.715c-11.422,0-20.715-9.293-20.715-20.715
				S368.948,391.941,380.37,391.941z" />
                                 <circle cx="255.999" cy="412.658" r="8.146" />
                                 <path d="M255.999,450.176c20.687,0,37.518-16.83,37.518-37.517c0-20.688-16.83-37.519-37.518-37.519
				c-20.687,0-37.517,16.83-37.517,37.519C218.481,433.345,235.312,450.176,255.999,450.176z M255.999,391.941
				c11.422,0,20.715,9.294,20.715,20.716s-9.293,20.715-20.715,20.715c-11.422,0-20.715-9.293-20.715-20.715
				S244.577,391.941,255.999,391.941z" />
                              </svg>
                           </div>
                           <h3 class="item-service__title">Городские фонтаны</h3>
                           <div class="item-service__text">Мы строим городские фонтаны</div>
                        </div>
                     </a>
                  </div>
                  <div class="services__column">
                     <a href="/walk/" class="services__link">
                        <div class="services__item item-service">
                           <div class="item-service__icon">
                              <svg version="1.1" id="Capa_1" fill="currentColor" alt="Пешеходный фонтан"
                                 xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
                                 y="0px" viewBox="0 0 434.85 434.85" xml:space="preserve">
                                 <path d="M427.35,300.368H269.81v-21.376c28.858-18.028,46.614-49.801,46.614-83.985c0-4.143-3.357-7.5-7.5-7.5H224.94v-10.491
										  c0.126-30.872,25.279-55.95,56.185-55.95c4.143,0,7.5-3.357,7.5-7.5s-3.357-7.5-7.5-7.5c-22.165,0-41.993,10.184-55.059,26.114
										  c6.581-40.319,41.643-71.196,83.795-71.196c13.223,0,25.887,2.952,37.64,8.774c3.713,1.84,8.212,0.32,10.05-3.392
										  c1.839-3.712,0.32-8.211-3.392-10.05c-13.841-6.855-28.745-10.332-44.298-10.332c-41.668,0-77.455,25.64-92.427,61.969
										  c-14.973-36.329-50.76-61.969-92.428-61.969c-15.552,0-30.456,3.477-44.298,10.333c-3.712,1.839-5.23,6.338-3.392,10.05
										  c1.838,3.71,6.336,5.231,10.05,3.392c11.755-5.823,24.418-8.775,37.64-8.775c42.153,0,77.216,30.878,83.797,71.199
										  c-13.066-15.931-32.894-26.116-55.06-26.116c-4.142,0-7.5,3.357-7.5,7.5s3.358,7.5,7.5,7.5c30.919,0,56.08,25.098,56.185,55.988
										  v10.452h-83.995c-4.142,0-7.5,3.357-7.5,7.5c0,34.191,17.753,65.967,46.606,83.99v21.37H7.5c-4.142,0-7.5,3.357-7.5,7.5v16.315
										  c0,8.693,5.218,16.184,12.684,19.521v37.663c0,4.143,3.358,7.5,7.5,7.5h394.48c4.143,0,7.5-3.357,7.5-7.5v-37.662
										  c7.467-3.337,12.686-10.829,12.686-19.522v-16.315C434.85,303.725,431.492,300.368,427.35,300.368z M133.77,202.507h167.317
										  c-2.39,26.545-17.353,50.611-40.375,64.48h-86.584C151.115,253.121,136.157,229.057,133.77,202.507z M180.04,281.988h74.77v18.38
										  h-74.77L180.04,281.988L180.04,281.988z M419.85,315.368v8.815c0,3.516-2.86,6.376-6.376,6.376H21.376
										  c-3.516,0-6.376-2.86-6.376-6.376v-8.815H419.85z M27.684,373.868v-28.309h379.48v28.309H27.684z" />
                                 <path
                                    d="M32.584,153.407c4.142,0,7.5-3.357,7.5-7.5c0-4.193,0.308-8.406,0.915-12.521c0.605-4.098-2.227-7.909-6.324-8.515
										  c-4.102-0.599-7.91,2.228-8.515,6.325c-0.714,4.838-1.076,9.787-1.076,14.711C25.084,150.049,28.442,153.407,32.584,153.407z" />
                                 <path d="M38.263,113.064c1.039,0.493,2.135,0.728,3.213,0.728c2.805,0,5.496-1.581,6.78-4.283
										  c3.793-7.983,8.8-15.31,14.881-21.774c2.838-3.017,2.693-7.764-0.324-10.602c-3.018-2.839-7.764-2.694-10.602,0.324
										  c-7.151,7.603-13.04,16.22-17.504,25.614C32.93,106.812,34.521,111.286,38.263,113.064z" />
                                 <path d="M114.004,137.523c3.738-3.734,7.968-6.919,12.57-9.465c3.624-2.005,4.937-6.568,2.932-10.193
										  c-2.005-3.624-6.568-4.938-10.193-2.932c-5.828,3.224-11.18,7.254-15.909,11.979c-2.931,2.927-2.933,7.676-0.006,10.606
										  c1.465,1.466,3.385,2.199,5.306,2.199C110.622,139.717,112.54,138.986,114.004,137.523z" />
                                 <path d="M97.554,177.248c0-6.037,0.95-11.973,2.824-17.644c1.299-3.933-0.835-8.175-4.769-9.474
										  c-3.933-1.304-8.175,0.834-9.475,4.769c-2.376,7.19-3.581,14.71-3.581,22.349c0,4.143,3.358,7.5,7.5,7.5
										  S97.554,181.39,97.554,177.248z" />
                                 <path
                                    d="M393.868,133.386c0.608,4.113,0.916,8.326,0.916,12.521c0,4.143,3.357,7.5,7.5,7.5s7.5-3.357,7.5-7.5
										  c0-4.926-0.362-9.875-1.076-14.712c-0.605-4.099-4.431-6.927-8.516-6.324C396.095,125.476,393.264,129.289,393.868,133.386z" />
                                 <path d="M386.611,109.508c1.284,2.702,3.975,4.283,6.779,4.283c1.079,0,2.175-0.234,3.214-0.728
										  c3.741-1.778,5.333-6.252,3.556-9.993c-4.464-9.396-10.354-18.014-17.504-25.614c-2.838-3.018-7.585-3.162-10.602-0.324
										  c-3.018,2.838-3.162,7.585-0.324,10.602C377.811,94.198,382.817,101.524,386.611,109.508z" />
                                 <path d="M305.361,117.865c-2.005,3.625-0.692,8.188,2.932,10.193c4.602,2.546,8.831,5.73,12.571,9.466
										  c1.464,1.462,3.382,2.193,5.3,2.193c1.921,0,3.842-0.733,5.307-2.2c2.927-2.931,2.924-7.679-0.007-10.606
										  c-4.729-4.725-10.083-8.755-15.909-11.978C311.928,112.928,307.366,114.242,305.361,117.865z" />
                                 <path d="M339.258,150.13c-3.933,1.3-6.067,5.542-4.768,9.475c1.873,5.669,2.823,11.604,2.823,17.643c0,4.143,3.357,7.5,7.5,7.5
										  s7.5-3.357,7.5-7.5c0-7.641-1.205-15.16-3.581-22.35C347.433,150.966,343.191,148.833,339.258,150.13z" />
                              </svg>
                             
                           </div>
                           <h3 class="item-service__title">Пешеходные фонтаны</h3>
                           <div class="item-service__text"><span>Мы строим пешеходные фонтаны</span></div>
                        </div>
                     </a>
                  </div>
                  <div class="services__column">
                     <a href="/private/" class="services__link">
                        <div class="services__item item-service">
                           <div class="item-service__icon">
                              <svg version="1.1" id="Capa_1" fill="currentColor" alt="Частный фонтан"
                                 xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
                                 y="0px" viewBox="0 0 511.971 511.971" xml:space="preserve">
                                 <path d="M439.986,239.971h-160V173.79c9.173-1.723,17.628-4.433,24.833-8.036c14.94-7.47,23.167-18.047,23.167-29.783
		c0-4.418-3.582-8-8-8h-56v-24h8c4.418,0,8-3.582,8-8s-3.582-8-8-8h-7.602c0.456-4.591,1.301-9.125,2.535-13.549
		c1.187-4.256-1.301-8.668-5.557-9.855c-1.857-0.518-3.743-0.336-5.378,0.386c-1.635-0.722-3.52-0.904-5.376-0.386
		c-4.256,1.187-6.744,5.599-5.557,9.855c1.234,4.424,2.079,8.958,2.535,13.549h-7.602c-4.418,0-8,3.582-8,8s3.582,8,8,8h8v24h-56
		c-4.418,0-8,3.582-8,8c0,11.736,8.228,22.313,23.167,29.783c7.205,3.603,15.66,6.313,24.833,8.036v66.182h-160
		c-4.418,0-8,3.582-8,8c0,21.054,20.907,41.177,58.871,56.662c30.822,12.572,68.892,20.421,109.129,22.666v16.672h-24
		c-4.418,0-8,3.582-8,8s3.582,8,8,8c8.822,0,16,7.178,16,16v88h-28c-6.617,0-12,5.383-12,12v28c0,4.418,3.582,8,8,8h128
		c4.418,0,8-3.582,8-8v-28c0-6.617-5.383-12-12-12h-28v-88c0-8.822,7.178-16,16-16c4.418,0,8-3.582,8-8s-3.582-8-8-8h-24V327.3
		c40.237-2.245,78.307-10.094,109.129-22.666c37.963-15.485,58.871-35.608,58.871-56.662
		C447.986,243.553,444.404,239.971,439.986,239.971z M203.723,143.971h104.526c-7.924,8.568-27.718,16-52.263,16
		S211.647,152.54,203.723,143.971z M247.986,175.729c2.64,0.155,5.308,0.243,8,0.243s5.36-0.088,8-0.243v64.243h-16V175.729z
		 M311.986,495.971h-112v-16h32h48h32V495.971z M271.986,375.971v88h-32v-88c0-5.827-1.573-11.289-4.305-16h40.61
		C273.559,364.682,271.986,370.145,271.986,375.971z M263.986,343.971h-16v-16.078c2.661,0.049,5.327,0.078,8,0.078
		s5.339-0.029,8-0.078V343.971z M383.072,289.819c-35.022,14.285-80.156,22.152-127.086,22.152s-92.064-7.867-127.086-22.152
		c-24.389-9.948-41.31-22.332-46.9-33.848h347.973C424.382,267.487,407.461,279.871,383.072,289.819z" />
                                 <path d="M117.557,31.85c1.585,0,3.187-0.47,4.583-1.448c5.976-4.187,12.481-7.53,19.338-9.937
		c4.169-1.464,6.362-6.029,4.898-10.198s-6.029-6.363-10.198-4.898c-8.235,2.891-16.047,6.904-23.219,11.929
		c-3.619,2.536-4.497,7.524-1.961,11.143C112.554,30.662,115.036,31.85,117.557,31.85z" />
                                 <path d="M217.161,32.865c5.745,4.483,10.854,9.727,15.187,15.583c1.569,2.121,3.987,3.243,6.438,3.243
		c1.653,0,3.32-0.511,4.751-1.569c3.552-2.627,4.302-7.637,1.674-11.189c-5.194-7.021-11.319-13.306-18.205-18.681
		c-3.483-2.719-8.511-2.099-11.229,1.384S213.678,30.146,217.161,32.865z" />
                                 <path d="M82.006,75.494c0.845,0.284,1.705,0.419,2.55,0.419c3.342,0,6.458-2.111,7.582-5.453
		c2.314-6.884,5.569-13.432,9.674-19.459c2.487-3.652,1.542-8.628-2.109-11.115c-3.652-2.488-8.629-1.543-11.116,2.109
		c-4.928,7.236-8.836,15.098-11.616,23.368C75.564,69.551,77.818,74.087,82.006,75.494z" />
                                 <path d="M170.066,15.998c7.31,0.187,14.529,1.36,21.459,3.491c0.783,0.241,1.575,0.355,2.353,0.355c3.42,0,6.587-2.211,7.645-5.651
		c1.298-4.224-1.073-8.699-5.296-9.998c-8.322-2.558-16.987-3.968-25.753-4.191c-4.4-0.115-8.088,3.376-8.201,7.793
		C162.16,12.214,165.649,15.885,170.066,15.998z" />
                                 <path d="M79.986,223.971c4.418,0,8-3.582,8-8v-4.565c0-4.418-3.582-8-8-8s-8,3.582-8,8v4.565
		C71.986,220.389,75.567,223.971,79.986,223.971z" />
                                 <path d="M79.986,187.406c4.418,0,8-3.582,8-8v-24c0-4.418-3.582-8-8-8s-8,3.582-8,8v24
		C71.986,183.825,75.567,187.406,79.986,187.406z" />
                                 <path d="M79.986,131.406c4.418,0,8-3.582,8-8v-24c0-4.418-3.582-8-8-8s-8,3.582-8,8v24
		C71.986,127.825,75.567,131.406,79.986,131.406z" />
                                 <path
                                    d="M439.986,155.406c0-4.418-3.582-8-8-8s-8,3.582-8,8v24c0,4.418,3.582,8,8,8s8-3.582,8-8V155.406z" />
                                 <path d="M431.986,203.406c-4.418,0-8,3.582-8,8v4.565c0,4.418,3.582,8,8,8s8-3.582,8-8v-4.565
		C439.986,206.988,436.404,203.406,431.986,203.406z" />
                                 <path
                                    d="M423.986,99.406v24c0,4.418,3.582,8,8,8s8-3.582,8-8v-24c0-4.418-3.582-8-8-8S423.986,94.988,423.986,99.406z" />
                                 <path d="M419.834,70.46c1.124,3.343,4.24,5.453,7.582,5.453c0.845,0,1.705-0.135,2.55-0.419c4.188-1.407,6.442-5.943,5.034-10.132
		c-2.78-8.27-6.688-16.132-11.616-23.368c-2.487-3.652-7.463-4.596-11.116-2.109c-3.651,2.487-4.596,7.463-2.109,11.115
		C414.265,57.029,417.52,63.576,419.834,70.46z" />
                                 <path d="M268.435,50.122c1.431,1.059,3.098,1.569,4.751,1.569c2.45,0,4.869-1.122,6.438-3.243
		c4.332-5.856,9.442-11.1,15.187-15.583c3.483-2.719,4.103-7.746,1.384-11.229c-2.719-3.483-7.746-4.103-11.229-1.384
		c-6.886,5.375-13.011,11.66-18.205,18.681C264.133,42.485,264.883,47.495,268.435,50.122z" />
                                 <path d="M370.494,20.466c6.856,2.407,13.362,5.75,19.338,9.937c1.397,0.979,2.998,1.448,4.583,1.448
		c2.521,0,5.002-1.188,6.559-3.41c2.535-3.618,1.657-8.607-1.961-11.143c-7.172-5.024-14.984-9.038-23.219-11.929
		c-4.169-1.464-8.734,0.729-10.198,4.898C364.132,14.436,366.325,19.002,370.494,20.466z" />
                                 <path d="M318.094,19.844c0.778,0,1.57-0.114,2.353-0.355c6.93-2.13,14.149-3.304,21.459-3.491c4.417-0.112,7.906-3.784,7.794-8.201
		c-0.112-4.417-3.776-7.917-8.201-7.793c-8.766,0.223-17.431,1.633-25.753,4.191c-4.223,1.298-6.594,5.774-5.296,9.998
		C311.507,17.633,314.673,19.844,318.094,19.844z" />
                              </svg>
                             
                           </div>
                           <h3 class="item-service__title">Частные фонтаны</h3>
                           <div class="item-service__text">Мы строим частные фонтаны</div>
                        </div>
                     </a>
                  </div>
               </div>
            </div>
         </section>
         <section class="page__intro intro">
            <div class="intro__container _container">
               <div class="intro__body">
                  <div class="intro__item" data-aos="fade-left">
                     <div class="intro__text">
                        <div class="intro__textUp _private">
                           <h2>Хотите создать атмосферу гармонии и уюта в своем саду?
                              <hr> Не сомневайтесь - классический
                              фонтан станет идеальным дополнением к вашему уголку природы, способным преобразовать
                              обычную садовую
                              зону в сказочное место.
                           </h2>
                        </div>
                        <section class="page__slider slider">
                           <div class="slider__container _container">
                              <div class="slider__slider_text">
                                 <div class="slider__item">
                                    <p><span>Эстетика</span>
                                       <hr> фонтаны способны добавить красоту и привлекательность в любой сад. Благодаря
                                       разнообразию дизайнов, стилей и материалов, фонтан может стать потрясающим
                                       декоротивным элементом, который повысит общую эстетическую привлекательность
                                       вашего открытого пространства.
                                    </p>
                                 </div>
                                 <div class="slider__item">
                                    <p><span>Гармония и спокойствие</span>
                                       <hr> известно, что звук текущей воды оказывает успокаивающее действие и может
                                       помочь создать расслабляющую атмосферу. Фонтан может издавать нежное и спокойное
                                       журчание, которое поможет замаскировать нежелательные шумы, такие как шум
                                       транспорта или шум соседей, и создаст более спокойную обстановку.
                                    </p>
                                 </div>
                                 <div class="slider__item">
                                    <p><span>Единение с природой</span>
                                       <hr> фонтан может привлечь птиц и других мелких животных в ваш сад, создав для
                                       них естественный оазис, где они могут пить, купаться и даже гнездиться. Это может
                                       помочь создать более яркую и разнообразную экосистему в вашем дворе.
                                    </p>
                                 </div>
                                 <div class="slider__item">
                                    <p><span>Легкий уход</span>
                                       <hr> в зависимости от конструкции и размера фонтана его относительно легко
                                       обслуживать. При регулярной очистке и надлежащем уходе фонтан может служить много
                                       лет, обеспечивая красивое и не требующее особого ухода дополнение к вашему саду.
                                    </p>
                                 </div>
                              </div>
                           </div>
                        </section>


                     </div>
                  </div>
               </div>
            </div>
         </section>
         <section class="page_whoweare whoweare _city">
            <div class="whoweare__container _container">
               <div class="whoweare__header header-block">
                  <h2 class="header-block__title">Фонтаны, которые мы можем построить
                     для Вас!
                  </h2>
                  <div class="header-block__subtitle">У нас вы можете купить, и заказать строительство частных фонтанов.
                     <br>
                     Проекты на выбор,
                     стоимость и сроки поставки уточняйте.
                  </div>
               </div>
         </section>


         <section class="page__slider slider">
            <div class="slider__container _container">
               <div class="slider__slider">
                  <div class="slider__item">
                     <img src="<?php echo get_template_directory_uri(); ?>/img/private/1.jpeg" alt="example_fountain">
                  </div>
                  <div class="slider__item">
                     <img src="<?php echo get_template_directory_uri(); ?>/img/private/2.jpeg" alt="example_fountain">
                  </div>
                  <div class="slider__item">
                     <img src="<?php echo get_template_directory_uri(); ?>/img/private/3.jpg" alt="example_fountain">
                  </div>
                  <div class="slider__item">
                     <img src="<?php echo get_template_directory_uri(); ?>/img/private/4.jpg" alt="example_fountain">
                  </div>
                  <div class="slider__item">
                     <img src="<?php echo get_template_directory_uri(); ?>/img/private/5.jpg" alt="example_fountain">
                  </div>
                  <div class="slider__item">
                     <img src="<?php echo get_template_directory_uri(); ?>/img/private/6.jpeg" alt="example_fountain">
                  </div>
                  <div class="slider__item">
                     <img src="<?php echo get_template_directory_uri(); ?>/img/private/7.jpeg" alt="example_fountain">
                  </div>

               </div>
            </div>
         </section>
         <section class="page__getintouch getintouch">
            <div class="getintouch__container _container">
               <div class="getintouch__header header-block">
                  <h2 class="header-block__title">Свяжитесь с нами!
                  </h2>
                  <div class="header-block__subtitle">Наши менеджеры будут рады ответить на все ваши вопросы и
                     предложить
                     наилучшие решения.</div>
               </div>
               <div class="getintouch__items">
                  <div class="getintouch__item item-getintouch">
                     <div class="item-getintouch__icon">
                        <img src="<?php echo get_template_directory_uri(); ?>/img/icons/Getintouch/phone.svg" alt="phone">
                     </div>
                     <div class="item-getintouch__emails">
                        <a href="tel:+79653459967" class="item-getintouch__email">8-965-345-99-67</a>
                     </div>
                     <div class="item-getintouch__label">Звоните</div>
                     <a href="tel:+79653459967" class="item-getintouch__button">Позвонить</a>
                  </div>
                  <div class="getintouch__item item-getintouch item-getintouch_active">
                     <div class="item-getintouch__icon">
                        <img src="<?php echo get_template_directory_uri(); ?>/img/icons/Getintouch/mail.png" alt="mail">
                     </div>
                     <div class="item-getintouch__emails">
                        <a href="mailto:layserg9@mail.ru" class="item-getintouch__email">layserg9@mail.ru</a>
                     </div>
                     <div class="item-getintouch__label">Пишите</div>
                     <a href="mailto:layserg9@mail.ru" class="item-getintouch__button">Написать</a>
                  </div>
                  <div class="getintouch__item item-getintouch">
                     <div class="item-getintouch__icon">
                        <img src="<?php echo get_template_directory_uri(); ?>/img/icons/Getintouch/telegramm.svg" alt="telegramm">
                     </div>
                     <div class="item-getintouch__emails">
                        <a href="tg://resolve?domain=lilostich1" class="item-getintouch__email">telegram</a>
                     </div>
                     <div class="item-getintouch__label">Пишите</div>
                     <a href="tg://resolve?domain=lilostich1" class="item-getintouch__button">Написать</a>
                  </div>
               </div>
            </div>
         </section>
         <section class="page__subscribe subscribe">
            <div class="subscribe__container _container">
               <div class="subscribe__body">
                  <div class="subscribe__text">
                     <h4 class="subscribe__title">Оставьте заявку и мы обязательно с вами свяжемся</h4>
                  </div>
                  <form action="#" id="form" class="subscribe__form">
                     <input id="formName" type="text" name="name" class="subscribe__first_name _req"
                        placeholder="Ваше имя">
                     <input id="formPhone" type="tel" name="phone" class="subscribe__phone_number _req"
                        placeholder="+7 (___) ___-____">
                     <button type="submit" class="subscribe__button">Отправить</button>

                  </form>
               </div>
            </div>
         </section>
      </main>
      <?php get_footer(); ?>